...
def get_send_time(asc_report, aschex):
    ...
    print(send_time_bytes)
    unpacked_values = struct.unpack_from('!HBBBBB', send_time_bytes)  # Unpack the bytes
    send_time = '%d%02d%02d%02d%02d%02d' % unpacked_values  # Format the unpacked values
...
def get_gps_time(asc_report, aschex):
    ...
    gps_time = '%d%02d%02d%02d%02d%02d%02d' % struct.unpack_from('!HBBBBBB', gps_time_bytes)
...
def get_output(asc_report, aschex):
    output_status = aschex[:len_output*len_byte]
    asc_report.append(output_status)
    print("Digital Output:", output_status)
    return output_status, len_output * len_byte

def get_input(asc_report, aschex):
    input_status = aschex[:len_input*len_byte]
    asc_report.append(input_status)
    print("Digital Input :", input_status)
    return input_status, len_input * len_byte
...
def get_battery_level(asc_report, aschex):
    battery_level = '%d' % int(aschex[:len_battery_level*len_byte], 16)
    asc_report.append(battery_level)
    return battery_level, len_battery_level * len_byte
